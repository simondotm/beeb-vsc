...
export class LineParser {
	...
	public Process(): void {
		...
		while ( this.AdvanceAndCheckEndOfLine() ) {
			...
			if (isalpha(this._line[this._column]) || this._line[this._column] == '_') {
				do {
					this._column++;
				} while (this._column < this._line.length &&
			...
			if (bIsSymbolAssignment) {
				...
				if (GlobalData.Instance.IsFirstPass() && this._sourceCode.GetForCount() === 0) {
					...
					if ( SymbolTable.Instance.IsSymbolDefined( symbolName ) ) {
						...
						const loc = { uri: this._sourceCode.GetURI(), range: {start: startLoc, end: endLoc} };
				...
				if (this._column < this._line.length && this._line[ this._column ] == ',' ) {
					...
					throw new AsmException.SyntaxError_UnexpectedComma( this._line, this._column );
				}
			...
			if ( isalpha(this._line[this._column]) || this._line[this._column] == '_' ) {
				...
				if ( macro !== undefined ) {
					...
					for ( let i = 0; i < macro.GetNumberOfParameters(); i++ ) {
						const paramName = macro.GetParameter(i) + this._sourceCode.GetSymbolNameSuffix();

						try {
							if (!SymbolTable.Instance.IsSymbolDefined(paramName)) {
								const value = this.EvaluateExpression();
								const loc = { uri: this._sourceCode.GetURI(), range: {start: { line: this._lineno, character: oldColumn }, end: { line: this._lineno, character: this._column }} };
					...
					const macroInstance = new MacroInstance( macro, this._sourceCode, 
	...
	private GetInstructionAndAdvanceColumn(requireDistinctOpcodes = true): integer {
	...
	private SkipStatement(): void {
		...
		if (this._column < this._line.length && (this._line[this._column] == '{' || this._line[this._column] == '}' || this._line[this._column] == ':')) {
			this._column++;
			...
			while (this._column < this._line.length && (bInQuotes || bInSingleQuotes || this.MoveToNextAtom(':;\\{}'))) {
				if (this._column < this._line.length && this._line[this._column] == '"' && !bInSingleQuotes) {
					...
					bInQuotes = !bInQuotes;
				} else if (this._column < this._line.length && this._line[this._column] == '\'') {
					if (bInSingleQuotes) {
						bInSingleQuotes = false;
					} else if (this._column + 2 < this._line.length && this._line[this._column + 2] == '\'' && !bInQuotes) {
	...
	public EvaluateExpression(bAllowOneMismatchedCloseBracket = false): number | string {
		// Reset stacks
		...
		while ( this.AdvanceAndCheckEndOfSubStatement(bracketCount == 0) )
		{
			if (expected == TYPE.VALUE_OR_UNARY)
			{
				...
				for (let i = 0; i < LineParser._gaUnaryOperatorTable.length; i++)
				{
					...
					for ( let j = 0; j < len; j++ )
					{
						if ( (this._column + j >= this._line.length) || (token[j] != this._line[this._column + j].toUpperCase()) )
						{
							bMatch = false;
							break;
						}
					...
					if (bMatch) {
						...
						if ( (len > 1) && token[len - 1] == '(' ){
							pendingCommaCount = LineParser._gaUnaryOperatorTable[matchedToken].parameterCount - 1;
							this._column--;
				...
				if (matchedToken == -1) {
					...
					if (this._valueStackPtr == MAX_VALUES) {
						throw new AsmException.SyntaxError_ExpressionTooComplex(this._line, this._column);
					}
				...
				if ( thisOp.handler !== '' ) {
						...
						if ( opHandler !== '' ) {
							if (ast !== undefined) { // should never be undefined but have to for type checking
								...
								if ( ast.type === ASTType.BinaryOp ) {
									...
									numParams = Math.max(1, this._operatorStack[this._operatorStackPtr].parameterCount); // unary operator
								}
								// pull value(s) off the stack and set as children to ast
								for ( let i = 0; i < 2; i++ ) {
									const tmp = this._ASTValueStack.pop();
					...
					if ( bFoundMatchingBracket ) {
						if (separator) {
							...
							if (this._operatorStack[this._operatorStackPtr].parameterCount == 0) {
								throw new AsmException.SyntaxError_ParameterCount(this._line, this._column - 1);
							}
						...
						if ( bAllowOneMismatchedCloseBracket ) {
							...
							// mismatched brackets
							throw new AsmException.SyntaxError_MismatchedParentheses(this._line, this._column - 1);
						}
					}
				}
			}
	...
	private SkipExpression( bracketCount: integer, bAllowOneMismatchedCloseBracket: boolean ): void {
		while ( this.AdvanceAndCheckEndOfSubStatement(bracketCount == 0) ) {
			if ( this._line[this._column] == '(' ) {
				bracketCount++;
			}
	...
	private GetValue(): [number | string, boolean] {
		...
		let isSymbol = false;
		[canParse, this._column, double_value] = this.ParseNumeric(this._line, this._column);
		if ( canParse ) {
			value = double_value;
		}
		else if ( this._column < this._line.length && this._line[ this._column ] == '*' ) {
			...
			// get char literal
			if ( this._line.length - this._column < 3 ||  this._line[ this._column + 2 ] != '\'' ) {
		...
		else if (this._column < this._line.length && ( isalpha( this._line[ this._column ] ) || this._line[ this._column ] == '_' ) ) {
			// get a symbol
			...
			if (symbolName == 'TIME$') {
				...
				const location: Location = {
					...
					range: {
						start: { line: this._lineno, character: this._column - symbolName.length },
						end: { line: this._lineno, character: this._column } 
					} 
				};
				let hasValue: boolean;
				[hasValue, value] = this._sourceCode.GetSymbolValue(symbolName, location);
				if ( !hasValue )
				{
					// symbol not known
					throw new AsmException.SyntaxError_SymbolNotDefined( this._line, oldColumn );
				}
				isSymbol = true;
			}
		}
	...
	private ParseNumeric(line: string, index: integer): [boolean, integer, number] {
		...
		if ( is_decimal_digit(line[index]) || line[index] == '.' || line[index] == '-' ) {
			...
			[have_digits, index, buffer] = this.CopyDigitsSkippingUnderscores(line, index, buffer);
			// Copy decimal point
			if ( index < line.length && line[index] == '.')
			{
				...
				[have_digits_post, index, buffer] = this.CopyDigitsSkippingUnderscores(line, index, buffer);
			}
				...
				( line[index + 1] == '+' || line[index + 1] == '-' || is_decimal_digit(line[index + 1]) ) )
			{
				...
				[have_digits_exp, index, buffer] = this.CopyDigitsSkippingUnderscores(line, index, buffer);
				if ( !have_digits_exp )
				{
	...
	private ParseInteger(line: string, index: integer, base: integer, max_digits: integer): [boolean, integer, integer] {
		const old_index = index;
	...
	private CopyDigitsSkippingUnderscores(line: string, index: integer, buffer: string): [boolean, integer, string] {
	...
	private HasAddressingMode ( instructionIndex: integer, mode: ADDRESSING_MODE ): boolean {
	...
	private HandleAssembler(instruction: integer): void {
		...
		if ( this._column < this._line.length && this._line[this._column].toUpperCase() == 'A' && this.HasAddressingMode(instruction, ADDRESSING_MODE.ACC) ){
		...
		if ( this._column < this._line.length && this._line[this._column] == '(' ) {
			...
			if (this._column < this._line.length && this._line[this._column] == ')') {
				...
				if (!this.AdvanceAndCheckEndOfStatement()) {
					...
					if (this.HasAddressingMode(instruction, ADDRESSING_MODE.IND16)) {
						...
						if ((value & 0xFF) == 0xFF) {
							...
							throw new AsmException.SyntaxError_6502Bug(this._line, oldColumn + 1);
						}
					...
					if ( value > 0xFF )
					{
						...
						throw new AsmException.SyntaxError_NotZeroPage( this._line, oldColumn + 1 );
					}
					if (value < 0) {
						throw new AsmException.SyntaxError_BadAddress(this._line, oldColumn + 1);
					}
				...
				if (this._column < this._line.length && this._line[this._column] == ',') {
					...
					this.Assemble2(instruction, ADDRESSING_MODE.INDY, value, this._column - 1);
					return;
				}
			...
			if ( this._column < this._line.length && this._line[this._column] == ',' ) {
				...
				if ( this.HasAddressingMode(instruction, ADDRESSING_MODE.IND16X) ) {
					...
					this.Assemble3(instruction, ADDRESSING_MODE.IND16X, value, this._column - 2);
					return;
				}
				...
				this.Assemble2(instruction, ADDRESSING_MODE.INDX, value, this._column - 2);
				return;
			}
		...
		try {
			...
			if ( this.HasAddressingMode( instruction, ADDRESSING_MODE.REL ) && GlobalData.Instance.IsFirstPass() ) {
				value = ObjectCode.Instance.GetPC();
			}
		}
		catch (e) {
		...
		if ( !this.AdvanceAndCheckEndOfStatement() ) {
			...
			if ( this.HasAddressingMode(instruction, ADDRESSING_MODE.REL) ) {
				...
				if (branchAmount < -128) {
					throw new AsmException.SyntaxError_BranchOutOfRange(this._line, oldColumn);
				}
				else if (branchAmount > 127) {
					throw new AsmException.SyntaxError_BranchOutOfRange(this._line, oldColumn);
				}
				else {
					this.Assemble2(instruction, ADDRESSING_MODE.REL, branchAmount & 0xFF, 0);
					return;
				}
			}
			...
			if (value < 0x100 && this.HasAddressingMode(instruction, ADDRESSING_MODE.ZP)) {
		...
		if ( this._column < this._line.length && this._line[this._column].toUpperCase() == 'X' ) {
			this._column++;
			if ( this.AdvanceAndCheckEndOfStatement() ) {
				// We were not expecting any more characters
			...
			if (value < 0x100 && this.HasAddressingMode(instruction, ADDRESSING_MODE.ZPX)) {
				this.Assemble2(instruction, ADDRESSING_MODE.ZPX, value, this._column - 1);
				return;
			}
			else if (this.HasAddressingMode(instruction, ADDRESSING_MODE.ABSX)) {
				this.Assemble3(instruction, ADDRESSING_MODE.ABSX, value, this._column - 1);
				return;
			}
			else {
		...
		if ( this._column < this._line.length && this._line[this._column].toUpperCase() == 'Y' ) {
			this._column++;
			if ( this.AdvanceAndCheckEndOfStatement() ) {
				// We were not expecting any more characters
				throw new AsmException.SyntaxError_BadIndexed(this._line, this._column);
			...
			if (value < 0x100 && this.HasAddressingMode(instruction, ADDRESSING_MODE.ZPY)) {
				this.Assemble2(instruction, ADDRESSING_MODE.ZPY, value, this._column - 1);
				return;
			}
			else if (this.HasAddressingMode(instruction, ADDRESSING_MODE.ABSY)) {
				this.Assemble3(instruction, ADDRESSING_MODE.ABSY, value, this._column - 1);
				return;
			}
			else {
	...
	private Assemble1(instructionIndex: integer, mode: ADDRESSING_MODE, registerPosition: integer): void {
	...
	private Assemble2(instructionIndex: integer, mode: ADDRESSING_MODE, value: integer, registerPosition: integer): void {
		const opcode = this.GetOpcode(instructionIndex, mode);
		try {
	...
	Assemble3(instructionIndex: integer, mode: ADDRESSING_MODE, value: integer, registerPosition: integer): void {
		const opcode = this.GetOpcode(instructionIndex, mode);
		try {
	...
	private FormatAssemblyTime(formatString: string) {
		...
		if ( ts.length > 256 ) {
			throw new AsmException.SyntaxError_TimeResultTooBig( this._line, this._column );
		}
		return ts;
	}
	...
	private HandleDefineLabel(): void {
		if (this._column >= this._line.length) {
			throw new AsmException.SyntaxError_InvalidSymbolName(this._line, this._column);
		}
		...
		if (target_level < this._sourceCode.GetInitialForStackPtr()) {
			throw new AsmException.SyntaxError_SymbolScopeOutsideMacro(this._line, initialColumn);
		}
		...
		for (let level = this._sourceCode.GetForLevel(); level > target_level; level--) {
			if (this._sourceCode.IsRealForLevel(level)) {
				throw new AsmException.SyntaxError_SymbolScopeOutsideFor(this._line, initialColumn);
			}
		}
		if ((this._column < this._line.length) && (isalpha(this._line[this._column]) || this._line[this._column] == '_')) {
			...
			const fullSymbolName = symbolName + this._sourceCode.GetSymbolNameSuffix(target_level);
			if (GlobalData.Instance.IsFirstPass()) {
				// only add the symbol on the first pass
				if (SymbolTable.Instance.IsSymbolDefined(fullSymbolName)) {
					...
					const loc = { uri: this._sourceCode.GetURI(), range: {start: { line: this._lineno, character: oldColumn }, end: { line: this._lineno, character: this._column }} };
					SymbolTable.Instance.AddSymbol(fullSymbolName, ObjectCode.Instance.GetPC(), loc, true);
				}
			}
			else {
	...
	private HandleStatementSeparator(): void { /* do nothing! */ }
		...
		while (true) {
			try {
				...
				if (!GlobalData.Instance.IsFirstPass() && !value) {
					while ((this._column < this._line.length) && isspace(this._line[this._column])) {
						this._column++;
					}
	...
	private HandleSave(): void {
		...
		const exec = args.ParseInt().AcceptUndef().Default(start).Range(0, 0xFFFFFF).Value() as number;
		const reload = args.ParseInt().Default(start).Range(0, 0xFFFFFF).Value() as number;
		args.CheckComplete();
		if (!saveParam.Found()) {
			if (GlobalData.Instance.GetOutputFile() != null) {
				...
				if (GlobalData.Instance.IsSecondPass()) {
					if (GlobalData.Instance.GetNumAnonSaves() > 0) {
						throw new AsmException.SyntaxError_OnlyOneAnonSave(this._line, saveParam.Column());
					}
					else {
				...
				throw new AsmException.SyntaxError_NoAnonSave(this._line, saveParam.Column());
			}
		}
	}
	...
	private HandleMapChar(): void {
		...
		if (!param3.Found()) {
			...
			for (let i = param1; i <= param2; i++) {
				ObjectCode.Instance.SetMapping(i, param3.Value() as number + i - param1);
			}
		}
	}
	...
	private HandlePutFileCommon(text: boolean): void {
		...
		const start = args.ParseInt().AcceptUndef().Range(0, 0xFFFFFF).Value() as number;
		const exec = args.ParseInt().AcceptUndef().Default(start).Range(0, 0xFFFFFF).Value() as number;
		args.CheckComplete();
	}
	...
	private HandleMacro(): void {
		...
		if (isalpha(this._line[this._column]) || this._line[this._column] == '_') {
			...
			if (GlobalData.Instance.IsFirstPass()) {
				if (MacroTable.Instance.Exists(macroName)) {
					throw new AsmException.SyntaxError_DuplicateMacroName(this._line, this._column);
				}
				this._sourceCode.GetCurrentMacro()!.SetName(macroName);
			}
		}
		else {
			throw new AsmException.SyntaxError_InvalidMacroName(this._line, this._column);
		}
		...
		while (this.AdvanceAndCheckEndOfStatement()) {
			...
			else if (isalpha(this._line[this._column]) || this._line[this._column] == '_') {
		...
		if (hasParameters && !expectComma) {
			throw new AsmException.SyntaxError_UnexpectedComma(this._line, this._column - 1);
		}
	...
	private HandleAsm(): void {
		...
		if (instruction < 0) {
			throw new AsmException.SyntaxError_MissingAssemblyInstruction(parser._line, parser._column);
		}
	}
	...
	private EvalArcSin(): void {
		...
		if (isNaN(this._valueStack[this._valueStackPtr - 1] as number)) {
			throw new AsmException.SyntaxError_IllegalOperation(this._line, this._column - 1);
		}
	}
	...
	private EvalDegToRad(): void {
		this._valueStack[this._valueStackPtr - 1] = this.StackTopNumber() * (Math.PI / 180.0);
	}
	private EvalRadToDeg(): void {
		this._valueStack[this._valueStackPtr - 1] = this.StackTopNumber() * (180.0 / Math.PI);
	}
	...
	private EvalTime(): void {
		this._valueStack[this._valueStackPtr - 1] = this.FormatAssemblyTime(this.StackTopString());
	}
	...
	private EvalChr(): void {
		...
		if ((ascii < 0) || (ascii > 255)) {
			throw new AsmException.SyntaxError_IllegalOperation(this._line, this._column);
		}
	...
	private EvalMid(): void {
		...
		this._valueStack[this._valueStackPtr - 1] = text.substring(index, index + length);
	}
	...
	private EvalRight(): void {
		...
		this._valueStack[this._valueStackPtr - 1] = text.substring(text.length - count);
	}
	private EvalString(): void {
		...
		const text = value2;
		if ((count < 0) || (count >= 0x10000) || (text.length >= 0x10000) || (count * text.length >= 0x10000)) {
			throw new AsmException.SyntaxError_IllegalOperation(this._line, this._column);
		}
	...
	private EvalDivide(): void {
		const values = this.StackTopTwoNumbers();
		if (values[1] == 0) {
			throw new AsmException.SyntaxError_DivisionByZero(this._line, this._column - 1);
		}
		this._valueStack[this._valueStackPtr - 2] = Math.floor(values[0] / values[1]);
		this._valueStackPtr--;
	}
	private EvalDiv(): void {
		...
		this._valueStack[this._valueStackPtr - 2] = Math.floor(values[0]) / Math.floor(values[1]);
		this._valueStackPtr--;
	}
	...
	private ArithmeticShiftRight(value: number, shift: number): number {
		...
		if (value < 0) {
			const bitcount = 8 * 4;
			const mask = Math.pow(2,bitcount) - 1 - (Math.pow(2,bitcount - shift) - 1);
			shifted |= mask;
		}
		return shifted;
	}
...
class Argument {
	...
	constructor(line: string, column: integer, state?: State, value?: number | string) {
		this._line = line;
		this._column = column;
		if (state === undefined) {
...
class ArgListParser {
	...
	ParseString(): Argument {
		if ( !this.ReadPending() )
		{
			return new Argument(this._lineParser._line, this._paramColumn, State.StateMissing);
		}
		...
		this._pending = false;
		return new Argument(this._lineParser._line, this._paramColumn, State.StateFound, this._pendingValue);
	}
	...
	ParseNumber(convert:(arg: number)=>number): Argument {
		...
		this._pending = false;
		return new Argument(this._lineParser._line, this._paramColumn, State.StateFound, convert(this._pendingValue));
	}

	public CheckComplete(): void {
		if ( this._pending )
		{
			throw new AsmException.SyntaxError_TypeMismatch(this._lineParser._line, this._lineParser._column);
		}
		if ( this._lineParser.AdvanceAndCheckEndOfStatement() )
		{
			throw new AsmException.SyntaxError_InvalidCharacter(this._lineParser._line, this._lineParser._column);
		}
	}
	...
	private MoveNext(): boolean {
		if (this._first) {
			...
			if (this._lineParser.AdvanceAndCheckEndOfStatement()) {
				...
				if (this._lineParser._column >= this._lineParser._line.length || this._lineParser._line[this._lineParser._column] != ',') {
