...
export class SymbolTable {
  ...
  public GetSymbols(): Map<string, SymbolData> {
    const filtered = new Map(
      Array.from(this._map.entries()).filter(
        ([_, v]) => v.GetLocation() !== noLocation,
      ),
    )
    return filtered
  }
    ...
    if (GlobalData.Instance.IsSecondPass()) {
      ...
      const identifier =
        (this._labelStack.length === 0
          ? ''
          : this._labelStack[this._labelStack.length - 1]._identifier) +
        '.' +
        symbol
      this._lastLabel = {
        _addr: addr,
    ...
    if (GlobalData.Instance.IsSecondPass()) {
      ...
      if (this._lastLabel!._addr !== addr) {
        ...
        this._lastLabel!._identifier =
          (this._labelStack.length === 0
            ? ''
            : this._labelStack[this._labelStack.length - 1]._identifier) + label
