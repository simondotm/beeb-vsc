...
export class SourceCode {
  ...
  Process() {
    ...
    while (this._lineNumber < this._lines.length) {
      // get line lsp approach?
  ...
  StartElIf(line: string, column: number): void {
    ...
    this._ifStack[this._ifStackPtr - 1].condition =
      !this._ifStack[this._ifStackPtr - 1].passed
  }
  ...
  AddFor(
    ...
    const loc = {
      ...
      range: {
        ...
        end: { line: lineno, character: column + varName.length },
      },
    }
    ...
    this._context.symbolTable.PushFor(
      ...
      this._uri,
      this._lineNumber,
      this._forStack[this._forStackPtr - 1].id,
    )
  }
  ...
  UpdateFor(line: string, column: number): void {
    ...
    if (
      ...
      if (
        thisFor.count > 1 ||
        (thisFor.count === 1 && thisFor.lineNumber !== this._lineNumber)
      ) {
