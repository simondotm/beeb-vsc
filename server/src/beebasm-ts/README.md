# TypeScript adaptation of BeebAsm

This is a modified copy of BeebAsm created to support the parsing of code within Visual Studio Code. Main modifications include:

* Converting to TypeScript for easy portability and integration with the vscode extension frameworks.
* Does not exit on the first error detected as we want all potential problems highlighted in the code editor
* Populates diagnostic information to pass back to the extension for reporting
* File structure changed (mostly moving LineParser logic within the same class instead of split across multiple files)
* Constructs an abstract syntax tree as the code is parsed for easy post-processing (e.g. hover information needs to know what is being hovered over)
* Symbol table changes to permanantly store declared symbols whereas the original code creates local variables within a macro temporarily then deletes them on exit but we need to know where everything is
* No outputing of code or other files naturally

The original README contents are kept below.
